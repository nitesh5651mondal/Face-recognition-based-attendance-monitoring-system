...
def initialize_db():
    ...
    cursor.execute("INSERT OR IGNORE INTO messages (id, message) VALUES (?, ?)", 
                   (1, 'Welcome to Face Recognition Based Attendance Monitoring System'))
    connection.commit()
    connection.close()

# Step 2: Fetch the welcome message from the database
...
# Step 3: Create the GUI to display the message
def create_interface():
    ...
    label = tk.Label(root, text=welcome_message, font=("Arial", 14), wraplength=350, justify="center")
    label.pack(pady=50)
    
    root.mainloop()

# Initialize the database and launch the interface
initialize_db()
create_interface()
############################################# IMPORTING ################################################
import tkinter as tk
...
def contact():
    mess._show(title='Contact us', message="Please contact us on : 'mondalnitesh2@gmail.com' ")
